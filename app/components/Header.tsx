      ...
      < div className="bg-white rounded shadow-lg py-5 px-7" >
        <nav className="flex justify-between">
          ...
          <ul className="hidden md:flex flex-auto space-x-2">
            {
              routes.map((title => (<Link key={title.id} className="focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-800 text-gray-600 border border-white bg-gray-50 cursor-pointer px-3 py-2.5 font-normal text-xs leading-3 shadow-md rounded"
                href={title.href}
              >{title.title}</Link>
              )))
            }
